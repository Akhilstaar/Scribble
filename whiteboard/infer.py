...
x = Convolution1D(64, 1, activation=keras.layers.LeakyReLU(
    alpha=o), input_shape=(num_points, 3))(input_points)
...
x = BatchNormalization()(x)
x = Dense(9, weights=[np.zeros([256, 9]), np.array(
    [1, 0, 0, 0, 1, 0, 0, 0, 1]).astype(np.float32)])(x)
input_T = Reshape((3, 3))(x)

# Forward net
g = Lambda(mat_mul, arguments={'B': input_T})(input_points)
g = Convolution1D(64, 1, input_shape=(num_points, 3),
                  activation=keras.layers.LeakyReLU(alpha=o))(g)
...
lab = ['heart', 'circle', 'diamond', 'hand',
       'leaf', 'like', 'moon', 'square', 'triangle']
